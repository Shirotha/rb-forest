...
impl<K: Ord, V> Tree<K, V>
{
    ...
    fn fix_insert(mut ptr: NodeIndex,
        ...
        unwrap! { (): loop {
            ...
            let grandparent = parent_node.parent?;
            // SAFETY: tree is balanced, so nodes on parent level cannot be null
            ptr = if tree[grandparent].children[1]? == parent {
                if is_left {
            ...
            where [(); 1 - I]:, [(); 1 - (1 - I)]:
        {
            ...
            unwrap! { (): {
                ...
                if sibling_node.is_red() {
                    ...
                    // SAFETY: tree is balanced, so nodes on parent level cannot be null
                    parent = tree[ptr].parent?;
                    // SAFETY: tree is balanced, so nodes on node level cannot be null
                    sibling = tree[parent].children[1 - I]?;
                }
                ...
                if !nephews[I].is_some_and( |nephew| tree[nephew].is_red() ) && is_black {
                    ...
                    let [sibling_node, parent_node] = tree.get_many_mut([sibling, parent])?;
